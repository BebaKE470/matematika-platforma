MathPlatform.registerModule({
  "id": "3-u04-h065",
  "teacher": {
    "placement": "3. ročník · Sterometria · Hodina 65: Kontrolná písomná práca",
    "prerequisites": [
      "učivo ZŠ o bodoch, priamkach, rovinách a základných telesách",
      "predchádzajúci tematický celok Stereometria – metrické úlohy",
      "priestorová orientácia a označenie kocky ABCDEFGH; pri rezoch aj učivo predchádzajúcich hodín tohto celku"
    ],
    "planContent": [
      "Tematický plán uvádza pre celý celok: „Základné pojmy geometrie v priestore; Vzájomné polohy priamok a rovín; Rezy kocky.“",
      "Téma hodiny podľa tematického plánu: „Kontrolná písomná práca“."
    ],
    "planPerformance": [
      "Tematický plán uvádza pre celý celok: „Poznať vzťahy v kocke. Vedieť sa priestorovo orientovať v kocke.“",
      "ŠkVP pre 3. ročník konkretizuje: riešiť úlohy o vzájomnej polohe priamok a rovín v kocke, orientovať sa v priestore kocky a riešiť úlohy na rez kocky rovinou."
    ],
    "goals": [
      "diagnosticky overiť priestorovú orientáciu, vzájomné polohy a rezy bez nahrádzania klasifikovanej písomnej práce",
      "presne používať geometrické pojmy a svoje rozhodnutie stručne zdôvodniť"
    ],
    "competencies": [
      "priestorová predstavivosť a geometrická reprezentácia",
      "matematická komunikácia a argumentácia",
      "analýza chyby a kontrola riešenia",
      "digitálna kompetencia pri krátkej interaktívnej diagnostike"
    ],
    "enrichment": [
      "Návrh maturitného štandardu je použitý iba ako obohatenie spôsobu práce: presná reprezentácia priestorovej situácie, argumentácia a kontrola. Neurčuje poradie ani rozsah učiva."
    ],
    "estimatedTime": "45 min hodina; platforma približne 15 min aktívnej práce",
    "flow": [
      "0–5 min: aktivácia predchádzajúcich vedomostí a priestorový náčrt",
      "5–15 min: výklad/objavovanie alebo ručná konštrukcia podľa témy",
      "15–25 min: práca vo dvojiciach alebo ručné úlohy s kockou",
      "25–40 min: približne 15 min interaktívna platforma – viac krátkych rozhodovacích a argumentačných aktivít",
      "40–45 min: spoločné zdôvodnenie chýb, zhrnutie a exit ticket mimo platformy"
    ],
    "diagnostics": [
      "či žiak rozhoduje podľa geometrických vlastností, nie iba podľa vzhľadu perspektívneho obrázka",
      "či rozlišuje rovnobežné a mimobežné priamky",
      "pri rezoch či kontroluje, že každá strana rezu leží v konkrétnej stene",
      "či dokáže stručne zdôvodniť rozhodnutie"
    ],
    "partialUse": [
      "Platforma je navrhnutá na približne 15 minút a obsahuje viac krátkych aktivít s okamžitou spätnou väzbou.",
      "Ručný náčrt, model kocky, konštrukcia rezu a argumentácia na papieri zostávajú súčasťou vyučovacej hodiny a mobil ich nenahrádza."
    ],
    "notebook": "Ak je v module teoretický zápis, žiak vidí presné hotové znenie. Konštrukčné úlohy rieši ručne do zošita."
  },
  "student": {
    "title": "Kontrolná písomná práca",
    "subtitle": "Téma 65 · Sterometria",
    "intro": "15-minútová interaktívna časť: diagnostika pripravenosti po písomnej práci. Rozhoduj podľa vlastností priestoru, nie iba podľa toho, ako obrázok vyzerá.",
    "estimatedTime": "cca 15 min",
    "activities": [
      {
        "id": "i",
        "type": "info",
        "phase": "PRECVIČ",
        "title": "Diagnostika po písomnej práci",
        "html": "<p><strong>Táto 15-minútová aktivita nenahrádza klasifikovanú kontrolnú písomnú prácu.</strong> Použi ju po odovzdaní písomky alebo ako následnú diagnostiku.</p><div class=\"logic-visual\"><svg viewBox=\"0 0 620 330\" role=\"img\" aria-label=\"Kocka ABCDEFGH\"><g fill=\"none\" stroke=\"currentColor\" stroke-width=\"3\"><path d=\"M120 110 L360 110 L470 55 L230 55 Z\"/><path d=\"M120 110 L120 270 L360 270 L360 110\"/><path d=\"M360 270 L470 215 L470 55\"/><path d=\"M120 270 L230 215 L470 215\" stroke-dasharray=\"7 7\" opacity=\".55\"/><path d=\"M230 55 L230 215\" stroke-dasharray=\"7 7\" opacity=\".55\"/></g><g fill=\"currentColor\" font-family=\"system-ui\" font-size=\"18\"><text x=\"98\" y=\"105\">A</text><text x=\"365\" y=\"105\">B</text><text x=\"480\" y=\"55\">C</text><text x=\"215\" y=\"45\">D</text><text x=\"98\" y=\"292\">E</text><text x=\"365\" y=\"292\">F</text><text x=\"480\" y=\"230\">G</text><text x=\"210\" y=\"235\">H</text></g></svg></div>"
      },
      {
        "id": "q0",
        "type": "choice",
        "phase": "PRECVIČ",
        "title": "Diagnostická úloha",
        "skill": "diagnostika",
        "question": "V kocke ABCDEFGH: Ktoré priamky sú mimobežné?",
        "options": [
          "AB a BF",
          "AB a EF",
          "AB a CG"
        ],
        "answer": 2,
        "correct": "Správne. Dôležité je vedieť rozhodnutie aj zdôvodniť."
      },
      {
        "id": "q1",
        "type": "choice",
        "phase": "PRECVIČ",
        "title": "Diagnostická úloha",
        "skill": "diagnostika",
        "question": "V kocke ABCDEFGH: Ktoré roviny sú rovnobežné?",
        "options": [
          "ABFE a DCGH",
          "ABFE a BCGF",
          "ABCD a ABFE"
        ],
        "answer": 0,
        "correct": "Správne. Dôležité je vedieť rozhodnutie aj zdôvodniť."
      },
      {
        "id": "q2",
        "type": "choice",
        "phase": "PRECVIČ",
        "title": "Diagnostická úloha",
        "skill": "diagnostika",
        "question": "V kocke ABCDEFGH: Prienikom dvoch rôznobežných rovín je…",
        "options": [
          "bod",
          "priamka",
          "úsečka vždy dĺžky hrany"
        ],
        "answer": 1,
        "correct": "Správne. Dôležité je vedieť rozhodnutie aj zdôvodniť."
      },
      {
        "id": "q3",
        "type": "choice",
        "phase": "PRECVIČ",
        "title": "Diagnostická úloha",
        "skill": "diagnostika",
        "question": "V kocke ABCDEFGH: Pri konštrukcii rezu môžeme spojiť dva známe body priamo vtedy, keď…",
        "options": [
          "majú rovnakú vzdialenosť od A",
          "sú ľubovoľné body kocky",
          "ležia na tej istej stene kocky"
        ],
        "answer": 2,
        "correct": "Správne. Dôležité je vedieť rozhodnutie aj zdôvodniť."
      },
      {
        "id": "q4",
        "type": "choice",
        "phase": "PRECVIČ",
        "title": "Diagnostická úloha",
        "skill": "diagnostika",
        "question": "V kocke ABCDEFGH: Ktoré tvrdenie je nepravdivé?",
        "options": [
          "Každý rez kocky je štvorec.",
          "Rez kocky je rovinný mnohouholník.",
          "Strany rezu ležia v stenách kocky."
        ],
        "answer": 0,
        "correct": "Správne. Dôležité je vedieť rozhodnutie aj zdôvodniť."
      },
      {
        "id": "q5",
        "type": "choice",
        "phase": "PRECVIČ",
        "title": "Diagnostická úloha",
        "skill": "diagnostika",
        "question": "V kocke ABCDEFGH: Ak sa dve priamky v priestore nepretínajú…",
        "options": [
          "sú vždy rovnobežné",
          "môžu byť rovnobežné alebo mimobežné",
          "sú vždy mimobežné"
        ],
        "answer": 1,
        "correct": "Správne. Dôležité je vedieť rozhodnutie aj zdôvodniť."
      },
      {
        "id": "q6",
        "type": "choice",
        "phase": "PRECVIČ",
        "title": "Diagnostická úloha",
        "skill": "diagnostika",
        "question": "V kocke ABCDEFGH: Tri body určujú rovinu jednoznačne, ak…",
        "options": [
          "ležia na jednej priamke",
          "sú ľubovoľné",
          "neležia na jednej priamke"
        ],
        "answer": 2,
        "correct": "Správne. Dôležité je vedieť rozhodnutie aj zdôvodniť."
      },
      {
        "id": "q7",
        "type": "choice",
        "phase": "PRECVIČ",
        "title": "Diagnostická úloha",
        "skill": "diagnostika",
        "question": "V kocke ABCDEFGH: Ktorá kontrola hotového rezu je najužitočnejšia?",
        "options": [
          "Overiť každú stranu v príslušnej stene a uzavretosť mnohouholníka.",
          "Overiť iba počet vrcholov.",
          "Overiť, či je obrazec pravidelný."
        ],
        "answer": 0,
        "correct": "Správne. Dôležité je vedieť rozhodnutie aj zdôvodniť."
      },
      {
        "id": "sw",
        "type": "selfWrite",
        "phase": "PRECVIČ",
        "title": "Čo by si obhájil?",
        "skill": "argumentácia",
        "prompt": "Vyber jednu odpoveď, pri ktorej by si vedel spolužiakovi vysvetliť, prečo je správna. Napíš stručný argument.",
        "model": "Argument má odkazovať na definíciu alebo vlastnosť, napríklad pri mimobežných priamkach na to, že sa nepretínajú, nie sú rovnobežné a neležia v jednej rovine."
      },
      {
        "id": "r",
        "type": "reflection",
        "phase": "ZÁVER",
        "title": "Po diagnostike",
        "prompt": "Označ, ako dobre tomu teraz rozumieš. Nie je to známka.",
        "skills": [
          "Vzájomné polohy mám isté.",
          "V priestore kocky sa orientujem.",
          "Postup rezu viem vysvetliť."
        ]
      }
    ]
  }
});
